...
<head>
  ...
  <style>
    :root {
      ...
      --strip-a: #1c2639;
      --strip-b: #212c41;
      --disabled-text: #5d6b80;
      --disabled-bg: #202a3d;
      --disabled-line: #202a3d;
    }
    :root[data-theme="light"] {
      ...
      --strip-a: #edf0f3;
      --strip-b: #e4e8ec;
      --disabled-text: #a2a9b2;
      --disabled-bg: #edf0f2;
      --disabled-line: #edf0f2;
    }
    ...
    * { box-sizing: border-box; }
    ...
    .theme-toggle:hover, .theme-toggle:focus-visible { color: var(--accent); border-color: var(--accent); outline: none; }
    ...
    .search:focus { border-color: var(--accent); box-shadow: 0 0 0 3px rgba(224, 82, 95, 0.2); }
    ...
    .catalog-card:hover, .catalog-card:focus-visible {
      ...
      box-shadow: 0 8px 20px rgba(224, 82, 95, 0.26);
      outline: none;
    ...
    .missing-media {
      ...
      line-height: 1.45;
      text-align: center;
      background: repeating-linear-gradient(45deg, var(--strip-a) 0, var(--strip-a) 9px, var(--strip-b) 9px, var(--strip-b) 18px);
    }
    ...
    .pager button:disabled { color: var(--disabled-text); cursor: not-allowed; }
    ...
    .detail-preview { display: grid; min-height: 600px; place-items: center; overflow: hidden; background: #0b1c34; }
    ...
    .actions button:disabled { color: var(--disabled-text); background: var(--disabled-bg); border-color: var(--disabled-line); cursor: not-allowed; }
  ...
  <main class="shell">
    <header class="topbar">
      ...
      <div class="top-actions">
        ...
        <button id="theme-toggle" class="theme-toggle" type="button" aria-label="切换深浅主题">浅色</button>
      </div>
    </header>
...
function readTheme() {
  try { return localStorage.getItem('svga-theme'); } catch { return null; }
}
function writeTheme(theme) {
  try { localStorage.setItem('svga-theme', theme); } catch { /* file:// 等受限环境忽略 */ }
}
function applyTheme(theme) {
  document.documentElement.dataset.theme = theme;
  const label = theme === 'dark' ? '浅色' : '深色';
  themeToggle.textContent = label;
  themeToggle.setAttribute('aria-label', '切换深浅主题（当前' + (theme === 'dark' ? '深色' : '浅色') + '）');
}
